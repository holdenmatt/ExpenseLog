...
class AppTests(unittest.TestCase):
    ...
    def test_summary(self):
        ...
        # Create a summary.
        res = self.post_json('/api/summary', data={
            'date': '2016-01-01',
            'summary': 'some text'
        })
        ...
        content = check_valid_json(res)
        eq_(content['id'], 1)
        eq_(content['date'], '2016-01-01')
        eq_(content['summary'], 'some text')

        # Modify it.
        res = self.put_json('/api/summary/1', data={
            'summary': 'new summary'
        })
        eq_(res.status_code, 200)

        # Get it.
        res = self.app.get('/api/summary/1')
        eq_(res.status_code, 200)
        content = check_valid_json(res)
        eq_(content['summary'], 'new summary')
        ...
        eq_(content['objects'][0]['summary'], 'new summary')

        # Delete it.
        res = self.app.delete('/api/summary/1')
    ...
    def test_summary_errors(self):
        ...
        eq_(res.status_code, 400)
        content = check_valid_json(res)
        eq_(content, {"message": "IntegrityError"})
